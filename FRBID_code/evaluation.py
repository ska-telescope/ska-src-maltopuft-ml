...
def model_prediction(
    ...
    name_file.write(
        "Balanced Accuracy: " + "%f" % float(balance_accuracy) + "\n"
    )
    name_file.write("=" * 80 + "\n")
    name_file.write("=" * 80 + "\n")
    ...
    name_file.write("=" * 80 + "\n")
    name_file.write(
        classification_report(y_test, ypred, target_names=classes) + "\n"
    )
    name_file.write("=" * 80 + "\n")
    name_file.write("=" * 80 + "\n")
    ...
    name_file.write("=" * 80 + "\n")
    name_file.write(
        classification_report_imbalanced(y_test, ypred, target_names=classes)
        + "\n"
    )
    name_file.write("=" * 80 + "\n")
    name_file.close()
...
def save_classified_examples(
    ...
    correctly_classified_bogus = pd.DataFrame(
        ID_bogus, columns=["transientid"]
    )
    ...
    if savecsv:
        ...
        ofname_real = os.path.join(odir_real)
        ensure_dir(ofname_real)
        correctly_classified_real.to_csv(
            ofname_real + "correctly_classified_frb.csv", index=None
        )

        ofname_bogus = os.path.join(odir_bogus)
        ensure_dir(ofname_bogus)
        correctly_classified_bogus.to_csv(
            ofname_bogus + "correctly_classified_rfi.csv", index=None
        )
    return (
