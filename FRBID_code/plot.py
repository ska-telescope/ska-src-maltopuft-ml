...
def optimsation_curve(history_, plot_dir1, plot_dir2, show=True):
    ...
    plt.plot(history_.history["accuracy"], c="r", lw=2.5, label="Training")
    plt.plot(
        history_.history["val_accuracy"], c="b", lw=2.5, label="validation"
    )
    ...
    for i, j in itertools.product(range(cm.shape[0]), range(cm.shape[1])):
        if normalize:
            ...
            plt.text(
                ...
                horizontalalignment="center",
                color=(
                    "white" if (cm[i, j] < 3) or (cm[i, j] >= 100) else "black"
                ),
                fontsize=18,
            )
...
def plot_images(data, ID, y_true, odir, savefig=False, show=True):
    ...
    for j in range(data.shape[0]):
        fig, axs = plt.subplots(
            1, data.shape[3], figsize=(15, 4), facecolor="w", edgecolor="k"
        )
